  ...
  return (
    <div className="fixed top-4 left-4 z-50 flex items-center gap-2">
      ...
      <div className="flex gap-1 bg-zinc-900/90 backdrop-blur border border-zinc-800 rounded-lg p-1">
        <button onClick={() => setLanguage('en')} className={`px-3 py-1 rounded text-sm font-medium transition-colors ${language === 'en' ? 'bg-violet-500 text-zinc-100' : 'text-zinc-400 hover:text-zinc-100'}`}>{t('langEn')}</button>
        <button onClick={() => setLanguage('es')} className={`px-3 py-1 rounded text-sm font-medium transition-colors ${language === 'es' ? 'bg-violet-500 text-zinc-100' : 'text-zinc-400 hover:text-zinc-100'}`}>{t('langEs')}</button>
      </div>
    </div>
  )
}
